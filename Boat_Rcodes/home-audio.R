...
#Charging_Duration <- Time required to charge the product
Charging_Duration <- character(length = len)

#NO_of_Drivers <- No. of drivers that the product have
...
home_audio$Warranty = as.numeric(substring(home_audio$Warranty , 1, 2))
...
#Cleaned data frame 
home_audio
save(home_audio, file = "home_audio.RData")
