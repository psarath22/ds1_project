...
file_path <- "wireless-headphones.txt"
...
wireless_headphones <- data.frame(ProductName = Titles,
...
#Data cleaning 
dims = dim(wireless_headphones)
n = dims[1] 

#Making Warranty in year numeric variable
wireless_headphones$Warranty = as.numeric(substring(wireless_headphones$Warranty , 1,2))

#Making Discount_percentage  numeric variable
foo = wireless_headphones$Discount_percentage 

foo = strsplit(foo , "%")

temp = as.numeric(sapply(1:n , function(k) substring(foo[[k]][1] , nchar(foo[[k]][1])-1 ,nchar(foo[[k]][1]))))

wireless_headphones$Discount_percentage = temp

wireless_headphones$Discount_percentage

#Making Regular_Price numeric variable  

foo = wireless_headphones$Regular_Price
...
wireless_headphones$Regular_Price = temp

#Making Sale_Price numeric variable

foo = wireless_headphones$Sale_Price
...
wireless_headphones$Sale_Price = temp 


#Making ReviewCount numeric variable
foo = wireless_headphones$ReviewCount
...
wireless_headphones$ReviewCount = temp

#MAking ProductRating numeric variable 
foo = wireless_headphones$ProductRating
...
wireless_headphones$ProductRating = temp

#Making Product Names clean
foo = wireless_headphones$ProductName
...
wireless_headphones$ProductName = temp

#New column for category
wireless_headphones$Category = "Wireless"

#New column to specify that they are wireless_headphones 
wireless_headphones$Type = "wireless_headphones"
...
temp <- wireless_headphones$Country_Of_Origin
temp[is.na(temp)] <- ""
temp -> wireless_headphones$Country_Of_Origin

#Charging Duration
#Changing Charging_Duration to numeric(in hrs)
temp <- wireless_headphones$Charging_Duration 
temp[is.na(temp)] <- ""
...
temp <- wireless_headphones$Charging_Time
temp[is.na(temp)] <- ""
...
temp <- as.numeric(temp)
as.numeric(temp) -> wireless_headphones$Charging_Time


#Cleaned data frame 
wireless_headphones 
save(wireless_headphones, file = "wireless_headphones.Rdata")